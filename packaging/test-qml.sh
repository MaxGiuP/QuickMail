...
for smoke in UiSmoke StoreContractSmoke PlainTextSecuritySmoke HtmlRenderSmoke MessageTextLayoutSmoke SenderAvatarSmoke ReaderAvatarLazySmoke WindowLifecycleSmoke SettingsWriteSmoke SettingsReadSmoke CalendarPaneSmoke
do
  ...
  if grep -Eq 'QML SMOKE TEST FAILED|UI SMOKE TEST FAILED|STORE CONTRACT TEST FAILED|PLAIN TEXT SECURITY TEST FAILED|HTML RENDER TEST FAILED|MESSAGE TEXT LAYOUT TEST FAILED|SENDER AVATAR TEST FAILED|READER AVATAR LAZY TEST FAILED|WINDOW LIFECYCLE TEST FAILED|SETTINGS PERSISTENCE TEST FAILED|CALENDAR PANE TEST FAILED|Binding loop detected|Type .* unavailable|Cannot assign to non-existent|ReferenceError:|is not defined|Failed to load component|Failed to load configuration' "$smoke_log"; then
    cat "$smoke_log" >&2
    rm -f "$smoke_log"
    exit 1
  fi
