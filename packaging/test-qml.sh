...
for smoke in UiSmoke StoreContractSmoke PlainTextSecuritySmoke HtmlRenderSmoke SettingsWriteSmoke SettingsReadSmoke
do
  ...
  if grep -Eq 'QML SMOKE TEST FAILED|UI SMOKE TEST FAILED|STORE CONTRACT TEST FAILED|PLAIN TEXT SECURITY TEST FAILED|HTML RENDER TEST FAILED|SETTINGS PERSISTENCE TEST FAILED|Binding loop detected|Type .* unavailable|Cannot assign to non-existent|ReferenceError:|is not defined|Failed to load component|Failed to load configuration' "$smoke_log"; then
    cat "$smoke_log" >&2
    rm -f "$smoke_log"
    exit 1
  fi
