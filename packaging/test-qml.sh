...
do
  ...
  if grep -Eq 'QML SMOKE TEST FAILED|UI SMOKE TEST FAILED|STORE CONTRACT TEST FAILED|PLAIN TEXT SECURITY TEST FAILED|HTML RENDER TEST FAILED|MESSAGE TEXT LAYOUT TEST FAILED|READER ZOOM TEST FAILED|THEME MODE TEST FAILED|SETTINGS MENU TEST FAILED|SENDER AVATAR TEST FAILED|READER AVATAR LAZY TEST FAILED|WINDOW LIFECYCLE TEST FAILED|COMPOSE FORMATTING TEST FAILED|SETTINGS PERSISTENCE TEST FAILED|CALENDAR PANE TEST FAILED|AGENDA COMPOSER LAYOUT TEST FAILED|Binding loop detected|Type .* unavailable|Cannot assign to non-existent|ReferenceError:|TypeError:|RangeError:|SyntaxError:|is not defined|Failed to load component|Failed to load configuration' "$smoke_log"; then
    cat "$smoke_log" >&2
    rm -f "$smoke_log"
    exit 1
  fi
